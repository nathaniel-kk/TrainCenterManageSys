...
use App\Http\Requests\TeachAddRequest;
...
class WriteController extends Controller{
    ...
    public function teachAdd(TeachAddRequest $request)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $id = 'B' . date("ymdis");
            $res=TeachingInspection::addId($id);
            $result = TeachingInspectionInfo::add($id, $data);
            return $result&&$res?
                json_success('成功!', $result, 200) :
                json_fail('失败!', null, 100);
    }
